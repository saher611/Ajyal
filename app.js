...
// --- إعداداتك الخاصة ---
const TELEGRAM_TOKEN = '7774187219:AAHOX_6uP97I6769p7RndOclN6u_eYt2L5c';
const SPREADSHEET_ID = '1coOeDXKCqgDLVrHBAwtIQ8hsDJQPED3oL1Jp-Ad7jmk';
const ULTRAMSG_INSTANCE = 'instance101905'; // تأكد من رقم النسخة لديك
const ULTRAMSG_TOKEN = 'your_ultramsg_token'; // ضع توكن Ultramsg هنا
...
app.post('/webhook', async (req, res) => {
    ...
    if (message && !message.fromMe) {
        ...
        // هنا يتم إنشاء Topic في تليجرام وإرسال الرسالة (يجب إضافة كود إنشاء التوبيك هنا)
        console.log(`رسالة من ${phone}: ${text}`);
    }
    res.sendStatus(200);
});
    ...
    if (topicId && replyText) {
        ...
        if (phone) {
            await axios.post(`https://api.ultramsg.com/${ULTRAMSG_INSTANCE}/messages/chat`, {
                token: ULTRAMSG_TOKEN, to: phone, body: replyText
            });
            ctx.reply("تم الإرسال لواتساب ✅");
        }
    }
});

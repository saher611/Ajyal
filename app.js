const express = require('express');
const axios = require('axios');
const { Telegraf } = require('telegraf');
const { google } = require('googleapis');

const app = express();
app.use(express.json());

// --- إعداداتك الخاصة (تأكد من صحتها 100%) ---
const TELEGRAM_TOKEN = '7774187219:AAHOX_6uP97I6769p7RndOclN6u_eYt2L5c'; // إذا استمر خطأ 401، أنشئ توكن جديد من BotFather
const SPREADSHEET_ID = '1coOeDXKCqgDLVrHBAwtIQ8hsDJQPED3oL1Jp-Ad7jmk';
const ULTRAMSG_INSTANCE = 'instance101905'; 
const ULTRAMSG_TOKEN = '689f9euh50m2l8d1'; // تأكد من التوكن الخاص بـ Ultramsg

const bot = new Telegraf(TELEGRAM_TOKEN);

// --- إعدادات جوجل من ملفك المرفق ---
const googleConfig = {
    email: "telegtram-whatsapp@enhanced-mote-381411.iam.gserviceaccount.com",
    key: "-----BEGIN PRIVATE KEY-----\\\\n-----END PRIVATE KEY-----\n",
    scopes: ['https://www.googleapis.com/auth/spreadsheets']
};

const auth = new google.auth.JWT(googleConfig.email, null, googleConfig.key, googleConfig.scopes);
const sheets = google.sheets({ version: 'v4', auth });

// وظائف الجدول
async function saveToSheet(topicId, phone) {
    try {
        await sheets.spreadsheets.values.append({
            spreadsheetId: SPREADSHEET_ID, range: 'Sheet1!A:B',
            valueInputOption: 'USER_ENTERED', resource: { values: [[topicId.toString(), phone.toString()]] }
        });
    } catch (e) { console.error("Sheets Save Error:", e.message); }
}

async function getPhoneFromSheet(topicId) {
    try {
        const res = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: 'Sheet1!A:B' });
        const rows = res.data.values;
        if (!rows) return null;
        const match = rows.reverse().find(row => row[0] == topicId.toString());
        return match ? match[1] : null;
    } catch (e) { return null; }
}

// استقبال رسائل تليجرام والرد لواتساب
bot.on('message', async (ctx) => {
    try {
        const topicId = ctx.message.message_thread_id;
        if (!topicId) return;

        const phone = await getPhoneFromSheet(topicId);
        if (phone && ctx.message.text) {
            await axios.post(`https://api.ultramsg.com/${ULTRAMSG_INSTANCE}/messages/chat`, {
                token: ULTRAMSG_TOKEN, to: phone, body: ctx.message.text
            });
            await ctx.reply("✅ تم الإرسال لواتساب", { reply_to_message_id: ctx.message.message_id });
        }
    } catch (e) { console.error("Telegram Process Error:", e.message); }
});

const PORT = process.env.PORT || 10000;
app.listen(PORT, () => {
    console.log(`Server is running on port ${PORT}`);
    bot.launch().catch(err => console.error("Failed to launch bot:", err.message));
});
